...
在Consul当中服务可以通过服务定义文件或者是HTTP API的方式进行注册。这里使用服务定义文件的方式将本地运行的node\_exporter通过服务的方式注册到Consul当中。
...
例如，可以使用node\_exporter.service.consul域名查询node\_exporter服务的信息：
...
如上所示DNS记录会返回当前可用的node\_exporter服务实例的IP地址信息。
...
![Consul UI](../../.gitbook/assets/consul_ui_page.png)
...
在consul\_sd\_configs定义当中通过server定义了Consul服务的访问地址，services则定义了当前需要发现哪些类型服务实例的信息，这里限定了只获取node\_exporter的服务实例信息。
